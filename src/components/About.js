...
const About = ({ lang }) => {
  ...
  let addr = document.createTextNode('here')
  url.appendChild(addr)
  url.setAttribute('onClick', () => window.open('http://php.tvermila.fi'))
  console.log('url', url)
  const en = `This site has been made with ReactJS. I also used React Router to move between sites and for styling Semantic UI React CSS framework.
  ...
  a small project for the PHP course at school. If you are interested, it can be found ${url}.`
  const fi = 'tättärää'
  const text = lang === 'en' ? en : fi
  return (
